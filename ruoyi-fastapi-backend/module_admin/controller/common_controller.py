...
async def common_upload(request: Request, file: UploadFile = File(...), overwrite: bool = False):
    upload_result = await CommonService.upload_service(request, file, overwrite)
    logger.info('上传成功')
